...
def run_pack(pack_name: str, target, delay: float = 1.0):
    ...
    print(f"Running pack: {pack_name} ({len(attacks)} attacks) against {target.name}")
    ...
    for idx, attack in enumerate(attacks):
        ...
        save_result(attack_id, category, prompt, response, error, verdict, severity, model=target.name)
